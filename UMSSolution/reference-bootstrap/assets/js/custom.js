(function ($) {
  ...
  $(document).ready(function () {
    // Initialize slick_slider_tab settings
    var slickTabSettings = {
      infinite: true,
      slidesToShow: 2,
      slidesToScroll: 1,
      responsive: [
    ...
    // Only initialize slick for visible tab sliders
    $(".tab-pane.active .slick_slider_tab").each(function() {
      if (!$(this).hasClass('slick-initialized')) {
        $(this).slick(slickTabSettings);
      }
    });

    // Initialize sliders that are NOT in tabs
    $(".slick_slider_tab").not(".tab-pane .slick_slider_tab").each(function() {
      if (!$(this).hasClass('slick-initialized')) {
        $(this).slick(slickTabSettings);
      }
    });

    // best_slider settings
    var bestSliderSettings = {
      infinite: true,
      slidesToShow: 4,
      slidesToScroll: 1,
      responsive: [
    ...
    // Only initialize best_slider for visible tabs
    $(".tab-pane.active .best_slider").each(function() {
      if (!$(this).hasClass('slick-initialized')) {
        $(this).slick(bestSliderSettings);
      }
    });

    // Initialize best_slider that are NOT in tabs
    $(".best_slider").not(".tab-pane .best_slider").each(function() {
      if (!$(this).hasClass('slick-initialized')) {
        $(this).slick(bestSliderSettings);
      }
    });
    ...
    // Tab switching - Initialize slick when tab becomes visible
    $('a[data-bs-toggle="pill"], button[data-bs-toggle="pill"]').on("shown.bs.tab", function (e) {
      // Get target tab - works for both <a href> and <button data-bs-target>
      var targetTab = $(e.target).attr("href") || $(e.target).attr("data-bs-target");

      // Small delay to ensure tab transition is complete
      setTimeout(function() {
        // Initialize slick_slider_tab for newly shown tab if not already initialized
        $(targetTab).find(".slick_slider_tab").each(function() {
          if (!$(this).hasClass('slick-initialized')) {
            $(this).slick(slickTabSettings);
          } else {
            // If already initialized, just refresh the position
            $(this).slick("setPosition");
          }
        });

        // Initialize best_slider for newly shown tab if not already initialized
        $(targetTab).find(".best_slider").each(function() {
          if (!$(this).hasClass('slick-initialized')) {
            $(this).slick(bestSliderSettings);
          } else {
            // If already initialized, just refresh the position
        ...
        // Also handle slick_slider_author if in tabs
        $(targetTab).find(".slick_slider_author").each(function() {
          if (!$(this).hasClass('slick-initialized')) {
            $(this).slick(slickTabSettings); // Using same settings as slick_slider_tab
          } else {
            $(this).slick("setPosition");
          }
        });
      }, 100); // 100ms delay to ensure tab is fully visible
    });
  });
