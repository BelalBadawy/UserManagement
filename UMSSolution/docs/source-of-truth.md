...
* **List & Traversal Flow**:
  ...
  2. On load, the page parses URL query parameters (`page`, `size`, `search`, `active`, `locked`, `role`, `sortBy`, `sortDir`) to initialize search, sorting, and filtering states reactively. User records, role lookups, and action mutations are managed via TanStack Query custom hooks (`useUserList`, `useAvailableRoles`, etc.) and cached accordingly, invalidating on mutation successes.
  ...
  4. The page dispatches a paginated request `GET /api/v1/users/paged-list` with `pageNumber`, `pageSize`, `sortBy`, `sortDirection`, `searchTerm`, `isActive`, `isLocked`, and `roleId`. Sorting keys and directions are fully synchronized to URL parameters. Changing any filter or search term automatically resets pagination back to Page 1.
  5. The footer renders a full pagination structure using the shared `<DataTablePagination />` component showing selection count, page size selection dropdown, and page numbers with ellipses. The pagination state is driven by the table instance (`useReactTable`) and synchronized to URL parameters.
  6. Parallel roles queries (`GET /api/v1/users/roles/{userId}`) are resolved in the background query handler with isolated `try/catch` blocks to fetch role mappings for rendering under the "Assigned Roles" column without crashing the core user list if an individual user role request fails.
  7. Column headers are interactive; clicking them triggers sorting dynamically, syncing to URL parameters and refetching from the API.

* **Guarded Action Operations**:
  1. **Lock/Unlock (Lockout)**: Governs security lockouts. Checked by `Permission.Identity.Users.Lock` / `Permission.Identity.Users.Unlock` claims. Action buttons show/hide based on the backend-tracked `IsLocked` (or `LockoutEnd`) status. Locking a user sets their lockout expiry date to a 1,000-year offset (`DateTimeOffset.UtcNow.AddYears(1000)`) to prevent database type overflows, immediately invalidates active refresh tokens (by setting their expiry to yesterday), and regenerates the user's **Security Stamp** (`UpdateSecurityStampAsync`). While the stateless JWT access token remains valid for its remaining short-lived duration (as the API does not query the database on every request), the security stamp update and refresh token invalidation ensure that any token refresh requests are rejected immediately, and the user is blocked from logging in or obtaining new access tokens, enforcing the lockout context. Unlocking resets the lockout date and resets the failed access count. Toggling triggers a confirmation modal and dispatches `PUT /api/v1/users/lock-user` or `PUT /api/v1/users/unlock-user` via `useLockUser` / `useUnlockUser` mutations.
  2. **Activate/Deactivate (Activation)**: Governs account status. Checked by `Permission.Identity.Users.Update`. Action buttons show/hide based on the `IsActive` flag. Deactivating a user flags the account as inactive in the database, preventing generic session logins. Toggling triggers a confirmation modal and dispatches `PUT /api/v1/users/change-status` via `useChangeUserStatus` mutation.
  3. **Create User**: Checked by `Permission.Identity.Users.Create`. Opens a side-panel `Sheet` allowing form entry. Evaluates password strength in real-time, blocking submission of weak passwords. Auto-assigns roles. Dispatches to `POST /api/v1/users/register` via `useRegisterUser` mutation.
  4. **Edit Details & Roles**: Checked by `Permission.Identity.Users.Update`. Loads existing credentials into `Sheet`. Admin edits Name, Phone Number, and toggles Assigned Roles. Clicking save dispatches profile updates (`PUT /api/v1/users/update`) and role assignment changes (`PUT /api/v1/users/user-roles`) sequentially in a single mutation lifecycle via the `useUpdateUserAndRoles` mutation hook.
  5. **Delete User**: Checked by `Permission.Identity.Users.Delete`. Deletion logic is verified and simulated client-side via `useDeleteUser` mutation.
...
* **List & Traversal Flow**:
  ...
  2. On load, the page parses URL query parameters (`page`, `size`, `search`, `status`, `sortBy`, `sortDir`) to initialize the table state reactively. Active records, lookups, and mutations are governed via TanStack Query custom hooks (`useCategoryList`, `useCategoryLookups`, etc.) and cached accordingly, invalidating the list cache automatically on mutations.
  3. The page displays categories (ID, Name, Slug, Sort Order, Status) in a Tanstack Table instance (`useReactTable`), with pagination rendered via a reusable `<DataTablePagination>` component that displays selections, page size selection dropdown, and page numbers with ellipses. The state of the pagination is fully driven by the table instance and synchronized back to URL parameters.
  4. Header columns are interactive; clicking them triggers TanStack Table sorting, which synchronizes to URL parameters and triggers an automatic background refetch from the API. A search input filters by name or slug. A status dropdown filters by Active, Inactive, or All.

* **Guarded Action Operations**:
  1. **Create Category**: Checked by `Permission.Product.Categories.Create`. Activates a side-panel `Sheet` containing Name, Slug (auto-generated from Name but editable), Parent Category (dynamically fetched dropdown of parent categories), Sort Order, and IsActive toggle. Submitting triggers `useCreateCategory` mutation and dispatches `POST /api/v1/categories`.
  2. **Edit Category**: Checked by `Permission.Product.Categories.Update`. Opens the `Sheet` populated with the category's current data. Submitting triggers `useUpdateCategory` mutation and dispatches `PUT /api/v1/categories` along with the `rowVersion` for concurrency control.
  3. **Delete Category**: Checked by `Permission.Product.Categories.Delete`. Displays a confirmation modal `Dialog`. On confirmation, triggers `useDeleteCategory` mutation and dispatches `DELETE /api/v1/categories/{id}`.
...
- **2026-06-01 (End of Day)**: Enhanced User Lockout Security.
  - **Security Stamp Invalidation**: Updated `UserService.LockUserAsync` to call `_userManager.UpdateSecurityStampAsync(user)` during lockouts, invalidating the user's security stamp. Combined with setting the refresh token's expiry to yesterday, this ensures that any subsequent token refresh requests are rejected immediately, preventing session continuation once the active short-lived JWT expires.
