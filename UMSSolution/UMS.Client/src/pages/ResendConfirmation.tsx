  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.isSuccessful) {
        setStatus('success')
        toast.success(result.messages?.[0] || 'Verification email sent successfully! Please check your inbox.')
      } else {
        setStatus('error')
        toast.error(result.messages?.[0] || 'Failed to resend verification email.')
      }
    } catch (err) {
      console.error(err)
      setStatus('error')
      ...
      <div className="relative z-10 w-full max-w-[500px] bg-white/80 backdrop-blur-xl rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-white/20 p-8 md:p-10 flex flex-col items-center text-center transition-all duration-300">
        ...
        <div className="w-full flex flex-col items-center space-y-6 my-4">
          ...
          <div className="text-center">
            <h3 className="text-xl font-bold text-neutral-800">Resend Verification Email</h3>
            <p className="text-neutral-500 text-xs mt-2 px-4 leading-relaxed">
              Enter the email address registered with your account, and we will send you a new confirmation link.
            </p>
          </div>
